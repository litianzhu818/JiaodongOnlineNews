...
@interface JDOReportActivityController () <ACTimeScrollerDelegate>
...
@implementation JDOReportActivityController{
...
-(void)loadView{
    ...
    _collectionView = [[PSUICollectionView alloc] initWithFrame:CGRectMake(0, Is_iOS7?64:44, 320, App_Height-(Is_iOS7?64:44)) collectionViewLayout:layout];
    ...
    __block JDOReportActivityController *blockSelf = self;
    ...
    self.publishBtn.frame = CGRectMake((320-53)/2, CGRectGetHeight(self.view.bounds)-53-20, 53, 53);
    ...
    _noDataView.frame = CGRectMake(0, -44, 320, CGRectGetHeight(self.view.bounds));
...
- (void)setupNavigationView{
    [self.navigationView setTitle:@"活动展示"];
    [self.navigationView addBackButtonWithTarget:self action:@selector(onBackBtnClick)];
    [self.navigationView addRightButtonImage:@"top_navigation_review" highlightImage:@"top_navigation_review" target:self action:@selector(showReadme)];
}

- (void) onBackBtnClick{
    JDOCenterViewController *centerViewController = (JDOCenterViewController *)[SharedAppDelegate deckController].centerController;
    [centerViewController popToViewController:[centerViewController.viewControllers objectAtIndex:centerViewController.viewControllers.count -2] orientation:JDOTransitionToRight animated:true];
}
...
- (void) refresh{
    if(![Reachability isEnableNetwork]){
        [JDOCommonUtil showHintHUD:No_Network_Connection inView:self.collectionView];
    ...
    [[JDOJsonClient sharedClient] getJSONByServiceName:IMAGE_DETAIL_SERVICE modelClass:@"JDOArrayModel" config:config params:@{@"aid":@"28924"} success:^(JDOArrayModel *dataModel) {
        ...
        [self.collectionView.pullToRefreshView stopAnimating];
        [JDOCommonUtil showHintHUD:errorStr inView:self.collectionView];
    }];
}
...
- (void) loadMore{
    ...
    [[JDOJsonClient sharedClient] getJSONByServiceName:IMAGE_DETAIL_SERVICE modelClass:@"JDOArrayModel" config:config params:@{@"aid":aid} success:^(JDOArrayModel *dataModel) {
        ...
        [self.collectionView.infiniteScrollingView stopAnimating];
        [JDOCommonUtil showHintHUD:errorStr inView:self.collectionView withSlidingMode:WBNoticeViewSlidingModeUp];
    }];
}
